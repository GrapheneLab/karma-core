...
struct get_impacted_account_visitor
{
   ...
   void operator()( const credit_request_operation& op ) 
   {
      _impacted.insert( op.borrower );
   }  

   void operator()( const credit_approve_operation& op ) 
   {
      _impacted.insert( op.creditor );
   }  

   void operator()( const credit_request_cancel_operation& op ) 
   {
      _impacted.insert( op.borrower );
   } 

   void operator()( const comment_credit_request_operation& op ) 
   {
      _impacted.insert( op.creditor );
   }  

   void operator()( const settle_credit_operation& op ) 
   {
      _impacted.insert( op.borrower );
   }  
